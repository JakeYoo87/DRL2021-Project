...
search_space = nasspace.get_search_space(args)
...
acc_list = np.zeros(len(search_space))

for i, (uid, network) in tqdm(enumerate(search_space), total=len(search_space)):
    try:
        network = network.to(device)
        data_iterator = iter(train_loader)
        x, target = next(data_iterator)
        x, target = x.to(device), target.to(device)
        acc_list[i] = search_space.get_final_accuracy(uid)
        # if i % 1000 == 0:
        #     print(i)
